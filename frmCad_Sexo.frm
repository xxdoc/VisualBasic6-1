...
Begin VB.Form frmCad_Sexo 
   ...
   Begin VB.Frame fraPainel 
      ...
      Begin VB.Frame fraDados 
         ...
         Top             =   1320
         Width           =   6615
         Begin MSFlexGridLib.MSFlexGrid mfgSexos 
            Height          =   2055
            Left            =   120
            TabIndex        =   6
            Top             =   360
...
Private Sub mfgSexos_Click()

    With mfgSexos

        .Col = 0
        If Len(Trim(.Text)) > 0 Then Call psMostrarSexo(CInt(Trim(.Text)))
...
Private Sub psCarregarSexos()

    On Error GoTo err_psCarregarSexos:
        ...
        With rs
            ...
            If Not .EOF Then
                ...
                While Not .EOF
                    ...
                    If pLinha > mfgSexos.Rows - 1 Then mfgSexos.Rows = mfgSexos.Rows + 1

                    mfgSexos.Row = pLinha

                    mfgSexos.Col = 0: mfgSexos.CellAlignment = 1
                    mfgSexos.Text = objBanco.gfsReadInt(.Fields("ID_SEXO"))
                    mfgSexos.Col = 1: mfgSexos.CellAlignment = 1
                    mfgSexos.Text = objBanco.gfsReadChar(.Fields("DE_SEXO"))
                    mfgSexos.Col = 2: mfgSexos.CellAlignment = 1
                    mfgSexos.Text = IIf(objBanco.gfsReadChar(.Fields("FL_ATIVO")) = "S", "Sim", "Não")
                ...
                mfgSexos.Row = 1: mfgSexos.Col = 0
                mfgSexos.ColSel = mfgSexos.Cols - 1
...
err_psCarregarSexos:
    Call objSystem.gsExibeErros(Err, "psCarregarSexos()", CStr(Me.Name))
...
Private Sub psExcluir()
    ...
    If objBanco.gfiExecuteSql(pSql) = -1 Then
        ...
        cn.RollbackTrans
        pMsg = "": pMsg = "Erro ao excluir o sexo!"
        MsgBox pMsg, vbOKOnly + vbCritical, "Atenção:"

    Else

        cn.CommitTrans
        pMsg = "": pMsg = "Sexo excluído com sucesso!"
...
Private Sub psMostrarSexo(pCodigo As Integer)

    On Error GoTo err_psMostrarSexo:
    ...
    With cmd
        ...
        With rs
            ...
            If Not .EOF Then
                ...
                pAcao = 2
                txtCodigo.Text = objBanco.gfsReadInt(.Fields("ID_SEXO"))
                txtDescricao.Text = objBanco.gfsReadChar(.Fields("DE_SEXO"))
...
err_psMostrarSexo:
    Call objSystem.gsExibeErros(Err, "psMostrarSexo()", CStr(Me.Name))
...
Private Sub psNovo()
    ...
    With Me
        ...
        .txtCodigo.Text = objBanco.gflProximoRegistro("tb_sexo")
...
Private Sub psSalvar()
    ...
    With Me
        ...
        If pAcao = 1 Then
            ...
            pSql = "": pSql = "CALL sp_ins_sexo"

        Else

            pSql = "": pSql = "CALL sp_upd_sexo"
    ...
    If objBanco.gfiExecuteSql(pSql) = -1 Then
        ...
        cn.RollbackTrans
        pMsg = "": pMsg = "Erro ao salvar o sexo!"
        MsgBox pMsg, vbOKOnly + vbCritical, "Atenção:"

    Else

        cn.CommitTrans
        pMsg = "": pMsg = "Sexo salvo com sucesso!"
        MsgBox pMsg, vbOKOnly + vbInformation, "Atenção:"
        Call psLimparCampos
...
Private Function pfbValidarCampos() As Boolean
    ...
    With Me
        ...
        If Len(Trim(.txtDescricao.Text)) = 0 Then
            ...
            pMsg = "": pMsg = "É necessário informar a descrição do sexo!"
            MsgBox pMsg, vbOKOnly + vbInformation, "Atenção:"
